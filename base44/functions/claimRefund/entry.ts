...
Deno.serve(async (req) => {
  try {
    ...
    console.log('[claimRefund] programId:', programIdStr);
    console.log('[claimRefund] Discriminator (hex):', discriminator.toString('hex'));

    // Accounts: market, bet_position, bettor [NOT signer], system_program
    const keys = [
    ...
    console.log('[claimRefund] Accounts:', keys.map((k, i) => `[${i}] ${k.pubkey}`));

    return Response.json({
      success: true, refundAmount: userBet.amount, userBetId,
      solana_instruction: {
