...
Deno.serve(async (req) => {
  try {
    ...
    // Step 1: Create a test match with timestamps in the FUTURE
    const now = Date.now();
    const matchTime = new Date(now + 10 * 60 * 1000); // 10 minutes from now
    const openUntil = new Date(now + 5 * 60 * 1000); // 5 minutes from now (betting closes)
    const settleAfter = new Date(now + 6 * 60 * 1000); // 6 minutes from now (settlement enabled)
    ...
    const bet = await base44.asServiceRole.entities.Bet.create({
      match_id: match.id,
      title: 'test1 vs test2 - Quick Test',
      outcome_a: 'test1',
      outcome_b: 'test2',
      outcome_draw: 'Draw',
    ...
    console.log('[quickTestMarket] Timestamps (WILL BE IN PAST BY TIME TX CONFIRMS):', {
