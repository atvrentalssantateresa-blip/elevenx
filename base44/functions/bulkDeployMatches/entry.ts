 ...
 * Creates Bet entities for matches without them, then prepares on-chain initialization instructions.
 */
Deno.serve(async (req) => {
  try {
    ...
    const matchesToDeploy = allMatches.filter(m => betByMatchId[m.id] && !betByMatchId[m.id].solana_market_created);
    ...
    // Step 2: Prepare on-chain instructions for all matches to deploy
    ...
    console.log(`[bulkDeployMatches] Prepared ${instructions.length} market instructions`);
    ...
    return Response.json({
      ...
      message: `Ready to deploy ${instructions.length} match markets to Solana`,
    });
